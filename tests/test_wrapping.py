...
def test_wrapping_jobrapido_xml_and_cpc(client: TestClient):
    """Job Rapido XML: official schema, DD/MM/YYYY dates, CPC, utm_source=jobrapido."""
    get_sess = list(app.dependency_overrides.values())[0]
    with next(get_sess()) as s:  # type: ignore
        row = models.JobrapidoJobFeed(
            ...
            postalcode=None,
            company="Acme",
            website="www.joinrs.com",
            publishdate=date(2026, 9, 1),
            expirydate=date(2026, 10, 31),
            salary="30000 EUR",
            education=None,
            jobtype="fulltime",
            category="Remote",
    ...
    assert "<country><![CDATA[IT]]></country>" in r.text
    assert "<website><![CDATA[www.joinrs.com]]></website>" in r.text
    assert "<publishdate><![CDATA[01/09/2026]]></publishdate>" in r.text
    assert "<expirydate><![CDATA[31/10/2026]]></expirydate>" in r.text
    assert "<cpc><![CDATA[" in r.text
    assert "0.08" in r.text.split("<cpc>")[1].split("</cpc>")[0]
    assert "<priority><![CDATA[1]]></priority>" in r.text
