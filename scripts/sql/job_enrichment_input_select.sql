-- Active jobs eligible for OpenAI enrichment (P1-3, Italy, skip list excluded).
-- Run on production DB (JOB_FEED_SOURCE_DATABASE_URL).

-- LinkedIn feed (GET /wrapping). Output columns match lw.job_postings (no auto id).
...
SELECT
    ...
    CONCAT(
        ...
        CASE
            ...
            ELSE ''
        END
    ) AS input_description,
    n.priority AS priority,
    n.has_ita AS has_ita,
    n.employers_id AS employers_id
FROM normalized n
WHERE
    n.has_ita = 1
    AND n.priority IN (1, 2, 3)
    AND n.employers_id NOT IN (829928, 1374217)
    AND (
